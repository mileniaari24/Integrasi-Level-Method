...
class page_controller{
    ...
    public function invoke()
    {
        ...
        $taik = json_encode($result);
        if(isset($_SERVER['HTTP_ACCEPT'])){ // yang minta browser
            $type=$_SERVER['HTTP_ACCEPT'];
           
            if(strpos($type,'text/html')!== false){
                if((!isset($_GET['id']))&&(!isset($_GET['nama']))&&(!isset($_GET['deskripsi'])))
                { // jika ada yang tidak terisi
                    $result=$this->dao_kat->select();
        ...
        if(isset($_SERVER['CONTENT_TYPE'])){ // selain browser
			...
			if(strpos($type1,'application/x-www-form-urlencoded')!== false){ //VB aplikasi
